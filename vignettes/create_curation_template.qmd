...
```{r}
## BioProjectID to SRA metadata
sra_per_bioproject <- bioproject_to_sra_metadata(bpid)
```

```{r}
names(sra_per_bioproject)
head(sra_per_bioproject)
head(sra_per_bioproject$sra_run_ids)
head(sra_per_bioproject$full_metadata)
```
...
```{r}
## BioProjectID to SRA metadata
sra_per_bioproject <- bioproject_to_sra_metadata(bioproject)
```
